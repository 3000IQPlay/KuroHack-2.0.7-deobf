...
public final class SnowUtil
{
    ...
    public static void updateModelViewProjectionMatrix() {
        GL11.glGetFloat(2982, SnowUtil.MODELVIEW);
        GL11.glGetFloat(2983, SnowUtil.PROJECTION);
        GL11.glGetInteger(2978, SnowUtil.VIEWPORT);
        final ScaledResolution res = new ScaledResolution(Minecraft.getMinecraft());
        GLUProjection.getInstance().updateMatrices(SnowUtil.VIEWPORT, SnowUtil.MODELVIEW, SnowUtil.PROJECTION, (double)(res.getScaledWidth() / (float)Minecraft.getMinecraft().displayWidth), (double)(res.getScaledHeight() / (float)Minecraft.getMinecraft().displayHeight));
    }
    ...
    public static void drawCircle(final int x, final int y, final int radius, final int color) {
        setupOverlayRendering();
        disableDefaults();
        GL11.glColor4d(getRedFromHex(color), getGreenFromHex(color), getBlueFromHex(color), getAlphaFromHex(color));
        GL11.glBegin(9);

...
package kurohack.util.kurohackutils;
...
public class KuroHackUtils implements Util
{
    ...
    public static void attackEntity(final Entity entity, final boolean packet, final boolean swingArm) {
        if (packet) {
            KuroHackUtils.mc.player.connection.sendPacket((Packet)new CPacketUseEntity(entity));
        }
        else {
            KuroHackUtils.mc.playerController.attackEntity((EntityPlayer)KuroHackUtils.mc.player, entity);
        }
        if (swingArm) {
            KuroHackUtils.mc.player.swingArm(EnumHand.MAIN_HAND);
        }
    }
    ...
    public static Vec3d getInterpolatedRenderPos(final Entity entity, final float partialTicks) {
        return getInterpolatedPos(entity, partialTicks).subtract(KuroHackUtils.mc.getRenderManager().renderPosX, KuroHackUtils.mc.getRenderManager().renderPosY, KuroHackUtils.mc.getRenderManager().renderPosZ);
    }
    
    public static Vec3d getInterpolatedRenderPos(final Vec3d vec) {
        return new Vec3d(vec.x, vec.y, vec.z).subtract(KuroHackUtils.mc.getRenderManager().renderPosX, KuroHackUtils.mc.getRenderManager().renderPosY, KuroHackUtils.mc.getRenderManager().renderPosZ);
    }
    ...
    public static boolean stopSneaking(final boolean isSneaking) {
        if (isSneaking && KuroHackUtils.mc.player != null) {
            KuroHackUtils.mc.player.connection.sendPacket((Packet)new CPacketEntityAction((Entity)KuroHackUtils.mc.player, CPacketEntityAction.Action.STOP_SNEAKING));
        }
        return false;
    }
    ...
    public static boolean isMobAggressive(final Entity entity) {
        if (entity instanceof EntityPigZombie) {
            ...
            if (entity instanceof EntityWolf) {
                return ((EntityWolf)entity).isAngry() && !KuroHackUtils.mc.player.equals((Object)((EntityWolf)entity).getOwner());
            }
    ...
    public static List<Vec3d> getUnsafeBlocksFromVec3d(final Vec3d pos, final int height, final boolean floor) {
        ...
        for (final Vec3d vector : getOffsets(height, floor)) {
            ...
            final Block block = KuroHackUtils.mc.world.getBlockState(targetPos).getBlock();
    ...
    public static boolean isObbyHole(final BlockPos blockPos) {
        ...
        for (final BlockPos pos : array) {
            final IBlockState touchingState = KuroHackUtils.mc.world.getBlockState(pos);
    ...
    public static double getDst(final Vec3d vec) {
        return KuroHackUtils.mc.player.getPositionVector().distanceTo(vec);
    }
    ...
    public static boolean isInWater(final Entity entity) {
        ...
        for (int x = MathHelper.floor(entity.posX); x < MathHelper.ceil(entity.posX); ++x) {
            for (int z = MathHelper.floor(entity.posZ); z < MathHelper.ceil(entity.posZ); ++z) {
                ...
                if (KuroHackUtils.mc.world.getBlockState(pos).getBlock() instanceof BlockLiquid) {
                    return true;
                }
            }
        }
    ...
    public static boolean isDrivenByPlayer(final Entity entityIn) {
        return KuroHackUtils.mc.player != null && entityIn != null && entityIn.equals((Object)KuroHackUtils.mc.player.getRidingEntity());
    }
    ...
    public static List<Vec3d> targets(final Vec3d vec3d, final boolean antiScaffold, final boolean antiStep, final boolean legs, final boolean platform, final boolean antiDrop, final boolean raytrace) {
        final ArrayList<Vec3d> placeTargets = new ArrayList<Vec3d>();
        if (antiDrop) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, KuroHackUtils.antiDropOffsetList));
        }
        if (platform) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, KuroHackUtils.platformOffsetList));
        }
        if (legs) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, KuroHackUtils.legOffsetList));
        }
        Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, KuroHackUtils.OffsetList));
        if (antiStep) {
            Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, KuroHackUtils.antiStepOffsetList));
        }
        else {
            ...
            if (vec3ds.size() == 4) {
                for (final Vec3d vector : vec3ds) {
                    ...
                    if (antiScaffold) {
                        Collections.addAll(placeTargets, BlockUtil.convertVec3ds(vec3d, KuroHackUtils.antiScaffoldOffsetList));
                    }
                    return placeTargets;
                }
            }
    ...
    public static boolean canEntityFeetBeSeen(final Entity entityIn) {
        return KuroHackUtils.mc.world.rayTraceBlocks(new Vec3d(KuroHackUtils.mc.player.posX, KuroHackUtils.mc.player.posX + KuroHackUtils.mc.player.getEyeHeight(), KuroHackUtils.mc.player.posZ), new Vec3d(entityIn.posX, entityIn.posY, entityIn.posZ), false, true, false) == null;
    }
    
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)KuroHackUtils.mc.player) || (entity instanceof EntityPlayer && KuroHack.friendManager.isFriend(entity.getName())) || KuroHackUtils.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static double getMaxSpeed() {
        ...
        if (KuroHackUtils.mc.player.isPotionActive((Potion)Objects.requireNonNull(Potion.getPotionById(1)))) {
            maxModifier *= 1.0 + 0.2 * (Objects.requireNonNull(KuroHackUtils.mc.player.getActivePotionEffect((Potion)Objects.requireNonNull(Potion.getPotionById(1)))).getAmplifier() + 1);
        }
        return maxModifier;
    }
    ...
    public static boolean isEntityMoving(final Entity entity) {
        ...
        if (entity instanceof EntityPlayer) {
            return KuroHackUtils.mc.gameSettings.keyBindForward.isKeyDown() || KuroHackUtils.mc.gameSettings.keyBindBack.isKeyDown() || KuroHackUtils.mc.gameSettings.keyBindLeft.isKeyDown() || KuroHackUtils.mc.gameSettings.keyBindRight.isKeyDown();
        }
    ...
    public static void moveEntityStrafe(final double speed, final Entity entity) {
        if (entity != null) {
            final MovementInput movementInput = KuroHackUtils.mc.player.movementInput;
            double forward = movementInput.moveForward;
            double strafe = movementInput.moveStrafe;
            float yaw = KuroHackUtils.mc.player.rotationYaw;
    ...
    public static boolean rayTraceHitCheck(final Entity entity, final boolean shouldCheck) {
        return !shouldCheck || KuroHackUtils.mc.player.canEntityBeSeen(entity);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && KuroHack.friendManager.isFriend((EntityPlayer)entity)) {
    ...
    public static boolean isMoving() {
        return KuroHackUtils.mc.player.moveForward != 0.0 || KuroHackUtils.mc.player.moveStrafing != 0.0;
    }
    
    public static EntityPlayer getClosestEnemy(final double distance) {
        EntityPlayer closest = null;
        for (final EntityPlayer player : KuroHackUtils.mc.world.playerEntities) {
            ...
            if (closest == null) {
                ...
                if (KuroHackUtils.mc.player.getDistanceSq((Entity)player) >= KuroHackUtils.mc.player.getDistanceSq((Entity)closest)) {
                    continue;
                }
                closest = player;
            }
    ...
    public static boolean checkCollide() {
        return !KuroHackUtils.mc.player.isSneaking() && (KuroHackUtils.mc.player.getRidingEntity() == null || KuroHackUtils.mc.player.getRidingEntity().fallDistance < 3.0f) && KuroHackUtils.mc.player.fallDistance < 3.0f;
    }
    
    public static BlockPos getPlayerPosWithEntity() {
        return new BlockPos((KuroHackUtils.mc.player.getRidingEntity() != null) ? KuroHackUtils.mc.player.getRidingEntity().posX : KuroHackUtils.mc.player.posX, (KuroHackUtils.mc.player.getRidingEntity() != null) ? KuroHackUtils.mc.player.getRidingEntity().posY : KuroHackUtils.mc.player.posY, (KuroHackUtils.mc.player.getRidingEntity() != null) ? KuroHackUtils.mc.player.getRidingEntity().posZ : KuroHackUtils.mc.player.posZ);
    }
    
    public static double[] forward(final double speed) {
        float forward = KuroHackUtils.mc.player.movementInput.moveForward;
        float side = KuroHackUtils.mc.player.movementInput.moveStrafe;
        float yaw = KuroHackUtils.mc.player.prevRotationYaw + (KuroHackUtils.mc.player.rotationYaw - KuroHackUtils.mc.player.prevRotationYaw) * KuroHackUtils.mc.getRenderPartialTicks();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : KuroHackUtils.mc.world.playerEntities) {
            if (!player.isInvisible()) {
                if (player.getName().equals(KuroHackUtils.mc.player.getName())) {
                    continue;
                }
                ...
                healthSB.append(hp);
                final int distanceInt = (int)KuroHackUtils.mc.player.getDistance((Entity)player);
                ...
                output.put(healthSB.toString() + " " + (KuroHack.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2§f0", (int)KuroHackUtils.mc.player.getDistance((Entity)player));

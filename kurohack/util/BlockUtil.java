...
public class BlockUtil implements Util
{
    ...
    public static boolean placeBlock(final BlockPos pos) {
        ...
        final boolean activated = block.onBlockActivated((World)Util.mc.world, pos, Util.mc.world.getBlockState(pos), (EntityPlayer)Util.mc.player, EnumHand.MAIN_HAND, direction, 0.0f, 0.0f, 0.0f);
        if (activated) {
        ...
        Util.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(pos.offset(direction), direction.getOpposite(), EnumHand.MAIN_HAND, 0.5f, 0.5f, 0.5f));
    ...
    public static EnumFacing calcSide(final BlockPos pos) {
        for (final EnumFacing side : EnumFacing.values()) {
            final IBlockState offsetState = Util.mc.world.getBlockState(pos.offset(side));
            final boolean activated = offsetState.getBlock().onBlockActivated((World)Util.mc.world, pos, offsetState, (EntityPlayer)Util.mc.player, EnumHand.MAIN_HAND, side, 0.0f, 0.0f, 0.0f);
            if (activated) {
                Util.mc.getConnection().sendPacket((Packet)new CPacketEntityAction((Entity)Util.mc.player, CPacketEntityAction.Action.START_SNEAKING));

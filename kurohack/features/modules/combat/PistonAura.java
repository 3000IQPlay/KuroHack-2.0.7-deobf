...
public class PistonAura extends Module
{
    ...
    public void onTick() {
        try {
            ...
            this.isTorch = true;
            if (this.redstoneType.getValue() == PistonAura.redstone.Block || (this.redstoneType.getValue() == PistonAura.redstone.Both && redstone == -1)) {
            ...
            this.debug_stage = 3;
            final boolean doTrap = this.trap.getValue() == mode.Force || (this.trap.getValue() == mode.Smart && KuroHack.holeManager.isSafe(PistonAura.mc.player.getPosition()) && this.b_piston.getY() == PistonAura.mc.player.getPosition().getY() + 1);
                ...
                if (this.getBlock(this.b_piston.add(0, -1, 0)).getBlock() == Blocks.AIR) {
                    ...
                    if (this.r_redstone) {
                        ...
                        if (!this.p_redstone) {
                            ...
                            this.timer = 0;
                            PistonAura.mc.player.inventory.currentItem = redstone;
                            PistonAura.mc.playerController.updateController();
    ...
    private void sp(final BlockPos offset, final int offset_y, final BlockPos enemy_pos) {
        ...
        if (this.isTorch) {
            ...
            pre_redstone = pre_redstone_place.stream().min(Comparator.comparing(b -> PistonAura.mc.player.getDistance((double)b.getX(), (double)b.getY(), (double)b.getZ()))).orElse(null);

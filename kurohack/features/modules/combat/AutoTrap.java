...
public class AutoTrap extends Module
{
    ...
    public AutoTrap() {
        ...
        this.antiScaffold = (Setting<Boolean>)this.register(new Setting("Speed", "AntiScaffold", 0.0, 0.0, (T)false, 0));
        this.antiStep = (Setting<Boolean>)this.register(new Setting("Speed", "AntiStep", 0.0, 0.0, (T)false, 0));
    ...
    private void doStaticTrap() {
        final List<Vec3d> placeTargets = KuroHackUtils.targets(this.target.getPositionVector(), this.antiScaffold.getValue(), this.antiStep.getValue(), false, false, false, this.raytrace.getValue());
    ...
    private EntityPlayer getTarget(final double range, final boolean trapped) {
        ...
        for (final EntityPlayer player : AutoTrap.mc.world.playerEntities) {
            if (!EntityUtil.isntValid((Entity)player, range) && (!trapped || !KuroHackUtils.isTrapped(player, this.antiScaffold.getValue(), this.antiStep.getValue(), false, false, false))) {
                if (KuroHack.speedManager.getPlayerSpeed(player) > 10.0) {
                    continue;
                }
                if (target == null) {
                    target = player;

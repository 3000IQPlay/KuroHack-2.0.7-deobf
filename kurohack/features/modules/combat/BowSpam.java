...
public class BowSpam extends Module
{
    ...
    public BowSpam() {
        ...
        this.tp = (Setting<Boolean>)this.register(new Setting("tp", (T)false, v -> this.mode.getValue() != Mode.BOWBOMB));
    ...
    public void onUpdateWalkingPlayer(final UpdateWalkingPlayerEvent event) {
        ...
        if (this.autoSwitch.getValue() && InventoryUtil.findHotbarBlock(ItemBow.class) != -1 && this.ownHealth.getValue() <= EntityUtil.getHealth((Entity)BowSpam.mc.player) && (!this.onlyWhenSave.getValue() || EntityUtil.isSafe((Entity)BowSpam.mc.player))) {
            ...
            if (target != null && (!(crystal = KuroHack.moduleManager.getModuleByClass(AutoCrystal.class)).isOn() || !InventoryUtil.holdingItem(ItemEndCrystal.class))) {
                ...
                KuroHack.rotationManager.lookAtVec3d(xPos, yPos, zPos);
                if (BowSpam.mc.player.getHeldItemMainhand().getItem() instanceof ItemBow) {
                    this.switched = true;
                }
            }
        ...
        if (this.mode.getValue() == Mode.FAST && (this.offhand || BowSpam.mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) && BowSpam.mc.player.isHandActive()) {
            ...
            final float f3 = this.tpsSync.getValue() ? KuroHack.serverManager.getTpsFactor() : 1.0f;
            if (f >= f2 * f3) {
    ...
    public void onUpdate() {
        ...
        switch (this.mode.getValue()) {
            case AUTORELEASE: {
                ...
                if (!this.timer.passedMs((int)(this.delay.getValue() * (this.tpsSync.getValue() ? KuroHack.serverManager.getTpsFactor() : 1.0f)))) {
                    break;
                }
    ...
    private EntityPlayer getTarget() {
        ...
        for (final EntityPlayer player : BowSpam.mc.world.playerEntities) {
            if (player != null && !EntityUtil.isDead((Entity)player) && EntityUtil.getHealth((Entity)player) <= this.health.getValue() && !player.equals((Object)BowSpam.mc.player) && !KuroHack.friendManager.isFriend(player) && BowSpam.mc.player.getDistanceSq((Entity)player) <= MathUtil.square(this.range.getValue())) {
    ...
    @Override
    public void onTick() {
        if (BowSpam.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemBow && BowSpam.mc.player.isHandActive() && BowSpam.mc.player.getItemInUseMaxCount() >= 20) {
            final double yaw = BowSpam.mc.player.rotationYaw * 0.017453292;
            final double power = this.tp.currentEnum();
            BowSpam.mc.player.connection.sendPacket((Packet)new CPacketPlayer.PositionRotation(BowSpam.mc.player.posX - Math.sin(yaw) * power, BowSpam.mc.player.posY + 5.0, BowSpam.mc.player.posZ + Math.cos(yaw) * power, BowSpam.mc.player.rotationYaw, BowSpam.mc.player.rotationPitch, false));
            BowSpam.mc.player.connection.sendPacket((Packet)new CPacketPlayerDigging(CPacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, BowSpam.mc.player.getHorizontalFacing()));
            BowSpam.mc.player.connection.sendPacket((Packet)new CPacketKeepAlive());

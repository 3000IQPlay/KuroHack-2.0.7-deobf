...
public class ModuleButton extends Button
{
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            ...
            KuroHack.textManager.drawStringWithShadow(((boolean)gui.openCloseChange.getValue()) ? (this.subOpen ? gui.close.getValue() : gui.open.getValue()) : gui.moduleButton.getValue(), this.x - 1.5f + this.width - 7.4f, this.y - 2.0f - CreepyWareGui.getClickGui().getTextOffset(), -1);
        }
        if (ClickGui.getInstance().gear.getValue()) {
            ModuleButton.mc.getTextureManager().bindTexture(this.logo);
            drawCompleteImage(this.x - 1.5f + this.width - 7.5f, this.y - 2.0f - CreepyWareGui.getClickGui().getTextOffset(), 9, 9);
        }
        if (this.subOpen) {
            float height = 1.0f;
